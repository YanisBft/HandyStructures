...
public class StructureCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher_1) {
		commandDispatcher_1.register((ServerCommandManager.literal("structure").requires((serverCommandSource_1) -> {
			...
			return loadStructure(context.getSource(), IdentifierArgumentType.getIdentifierArgument(context, "name"), BlockPosArgumentType.getPosArgument(context, "pos"), 0, "none");
		}).then(ServerCommandManager.argument("rotation", IntegerArgumentType.integer()).suggests(ROTATION_SUGGESTIONS).executes((context) -> {
			return loadStructure(context.getSource(), IdentifierArgumentType.getIdentifierArgument(context, "name"), BlockPosArgumentType.getPosArgument(context, "pos"), IntegerArgumentType.getInteger(context, "rotation"), "none");
		}).then(ServerCommandManager.argument("mirror", StringArgumentType.string()).suggests(MIRROR_SUGGESTIONS).executes((context) -> {
			return loadStructure(context.getSource(), IdentifierArgumentType.getIdentifierArgument(context, "name"), BlockPosArgumentType.getPosArgument(context, "pos"), IntegerArgumentType.getInteger(context, "rotation"), StringArgumentType.getString(context, "mirror"));
			...
			return saveStructure(context.getSource(), BlockPosArgumentType.getPosArgument(context, "from"), BlockPosArgumentType.getPosArgument(context, "to"), IdentifierArgumentType.getIdentifierArgument(context, "name"));
		}))))));
	}

	public static int loadStructure(ServerCommandSource source, Identifier name, BlockPos pos, int rot, String mir) throws CommandSyntaxException {
		int int_1 = 0;
		...
		if (!world.isClient && name != null) {
			...
			StructurePlacementData placementData = (new StructurePlacementData()).setMirrored(mirror).setRotation(rotation).setIgnoreEntities(true).setChunkPosition((ChunkPos)null);
	...
	public static int saveStructure(ServerCommandSource source, BlockPos from, BlockPos to, Identifier name) throws CommandSyntaxException {
		int int_1 = 0;		
		World world = source.getWorld();
